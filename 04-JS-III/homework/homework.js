...
function incrementarPorUno(array) {
 ...
 const arrayAumentado = []
  for (i = 0; i < array.length; i++) {arrayAumentado.push(array[i] + 1)}
...
function dePalabrasAFrase(palabras) {
  ...
  let fraseCompleta = "";
    for (i = 0; i < frase.length; i++) {fraseCompleta = fraseCompleta + frase[i]}
...
function cuentoElementos(arreglo){
  ...
  for (i = 0; i < arreglo.length; i++){
    if(arreglo[i] > 18){cantidadDeElementos++}
  }
  ...
  for(i = 0; i < array.length; i++) {
    ...
    if(meses.length < 3){
      return "No se encontraron los meses pedidos";}
        else{return meses;} USANDO UN FOR*/
